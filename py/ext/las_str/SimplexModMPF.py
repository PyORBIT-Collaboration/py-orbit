...
class Simplex: 
    def __init__(self, testfunc, guess, increments,kR = mpf(-1), kE = mpf(2), kC = mpf(1)/mpf(2)):
        ...
        self.lowest = mpf(-1)
        self.highest = mpf(-1)
        self.secondhighest = mpf(-1)

        self.errors = []
        self.currenterror = mpf(0)
        ...
        for vertex in range(0, self.numvars + 1):
            ...
            self.errors.append(mpf(0))
        self.calculate_errors_at_vertices()
        

    def minimize(self, epsilon = mpf(1)/mpf(10000), maxiters = 250, monitor = 1):
        ...
        for iter in range(0, maxiters):
            ...
            F2 = S / (self.numvars + mpf(1))

            #   compute the std deviation of the merit figures (ugly)

            S1 = mpf(0)
            for vertex in range(0, self.numvars + 1):
                S1 = S1 + (self.errors[vertex] - F2)**mpf(2)
    ...
    def contract_simplex(self):
        for x in range(0, self.numvars):
            self.guess[x] = self.kC * self.simplex[self.highest][x] + (mpf(1) - self.kC) * self.simplex[self.numvars + 1][x]
        return
    ...
    def expand_simplex(self):
        for x in range(0, self.numvars):
            self.guess[x] = self.kE * self.guess[x]                 + (mpf(1) - self.kE) * self.simplex[self.numvars + 1][x]
        return
    ...
    def reflect_simplex(self):
        # loop over variables
        for x in range(0, self.numvars):
            self.guess[x] = self.kR * self.simplex[self.highest][x] + (mpf(1) - self.kR) * self.simplex[self.numvars + 1][x]
    ...
    def multiple_contract_simplex(self):
        for vertex in range(0, self.numvars + 1):
            if vertex == self.lowest:
                continue
            for x in range(0, self.numvars):
                self.simplex[vertex][x] = mpf(1)/mpf(2) * (self.simplex[vertex][x] + self.simplex[self.lowest][x])
        self.calculate_errors_at_vertices()
        return
