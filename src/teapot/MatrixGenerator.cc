...
//
// DESCRIPTION
//   Generates linear transport matrix from bunch tracking.
//   Solves the system of equations for a,b,c:
//   y0 = a*x0^2 + b*x0 + c
//   y1 = a*x1^2 + b*x1 + c
//   y2 = a*x2^2 + b*x2 + c
...
namespace teapot_base
{
    ...
    void MatrixGenerator::calculateMatrix(Bunch* bunch,Matrix* mtrx)
    {
        ...
        {
            ORBIT_MPI_Finalize("MatrixGenerator::calculateMatrix: Matrix is the wrong size.");
        }
        if(bunch->getSize() != 13)
        {
    ...
    void MatrixGenerator::initBunchForChromaticityCoeff(Bunch* bunch)
    {
        ...
        bunch->addParticle(step_arr[0]/step_reduce, 0., 0., 0., 0., 0.);
        bunch->addParticle(step_arr[0]/step_reduce, 0., 0.,
                           0., 0., step_arr[5]/step_reduce);
        bunch->addParticle(step_arr[0]/step_reduce, 0., 0.,
                           0., 0., step_arr[5]);
        bunch->addParticle(step_arr[0], 0., 0., 0., 0., 0.);
        bunch->addParticle(step_arr[0], 0., 0.,
                           0., 0., step_arr[5]/step_reduce);
        bunch->addParticle(step_arr[0], 0., 0., 0., 0., step_arr[5]);
        //for xp-dE coefficient
        bunch->addParticle(0., step_arr[1]/step_reduce, 0., 0., 0., 0.);
        bunch->addParticle(0., step_arr[1]/step_reduce, 0.,
                           0., 0., step_arr[5]/step_reduce);
        bunch->addParticle(0., step_arr[1]/step_reduce, 0.,
                           0., 0., step_arr[5]);
        bunch->addParticle(0., step_arr[1], 0., 0., 0., 0.);
        bunch->addParticle(0., step_arr[1], 0.,
                           0., 0., step_arr[5]/step_reduce);
        bunch->addParticle(0., step_arr[1], 0., 0., 0., step_arr[5]);
        //for y-dE coefficient
        bunch->addParticle(0., 0., step_arr[2]/step_reduce, 0., 0., 0.);
        bunch->addParticle(0., 0., step_arr[2]/step_reduce,
                           0., 0., step_arr[5]/step_reduce);
        bunch->addParticle(0., 0., step_arr[2]/step_reduce,
                           0., 0., step_arr[5]);
        bunch->addParticle(0., 0., step_arr[2], 0., 0., 0.);
        bunch->addParticle(0., 0., step_arr[2],
                           0., 0., step_arr[5]/step_reduce);
        bunch->addParticle(0., 0., step_arr[2], 0., 0., step_arr[5]);
        //for yp-dE coefficient
        bunch->addParticle(0., 0., 0., step_arr[3]/step_reduce, 0., 0.);
        bunch->addParticle(0., 0., 0., step_arr[3]/step_reduce,
                           0., step_arr[5]/step_reduce);
        bunch->addParticle(0., 0., 0.,
                           step_arr[3]/step_reduce, 0., step_arr[5]);
        bunch->addParticle(0., 0., 0., step_arr[3], 0., 0.);
                                                     ...
                                                     double& coeff_yp_dE)
    {
        ...
        y1 = coord_arr[1][0];
        y2 = coord_arr[2][0];
        double coeff_x_dE_0  = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        y0 = coord_arr[0][1];
        y1 = coord_arr[1][1];
        y2 = coord_arr[2][1];
        double coeff_xp_dE_0 = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        y0 = coord_arr[0][2];
        y1 = coord_arr[1][2];
        y2 = coord_arr[2][2];
        double coeff_y_dE_0  = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        y0 = coord_arr[0][3];
        y1 = coord_arr[1][3];
        y2 = coord_arr[2][3];
        double coeff_yp_dE_0 = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        //calculation of x-dE coefficients
        y0 = coord_arr[3][0];
        y1 = coord_arr[4][0];
        y2 = coord_arr[5][0];
        double coeff_x_dE_1  = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        y0 = coord_arr[6][0];
        y1 = coord_arr[7][0];
        y2 = coord_arr[8][0];
        double coeff_x_dE_2  = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        //calculation of xp-dE coefficients
        y0 = coord_arr[9][1];
        y1 = coord_arr[10][1];
        y2 = coord_arr[11][1];
        double coeff_xp_dE_1 = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        y0 = coord_arr[12][1];
        y1 = coord_arr[13][1];
        y2 = coord_arr[14][1];
        double coeff_xp_dE_2 = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        //calculation of y-dE coefficients
        y0 = coord_arr[15][2];
        y1 = coord_arr[16][2];
        y2 = coord_arr[17][2];
        double coeff_y_dE_1  = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        y0 = coord_arr[18][2];
        y1 = coord_arr[19][2];
        y2 = coord_arr[20][2];
        double coeff_y_dE_2  = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        //calculation of yp-dE coefficients
        y0 = coord_arr[21][3];
        y1 = coord_arr[22][3];
        y2 = coord_arr[23][3];
        double coeff_yp_dE_1 = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        y0 = coord_arr[24][3];
        y1 = coord_arr[25][3];
        y2 = coord_arr[26][3];
        double coeff_yp_dE_2 = ((y1-y0)*x2*x2 - (y2-y0)*x1*x1) /
        (x1*x2*(x2-x1));
        //----------------------------------------
