...
void RungeKuttaTracker::trackBunch(Bunch* bunch, BaseFieldSource* fieldSource, ExternalEffects* extEff){
    ...
    for(int iN = 1; iN < 20; iN++){
        //std::cerr<<"debug start loop iN=" <<iN<<" time step="<<t_st<<"  nMax="<<nMax<<std::endl;
        nMax = nMax*2;
        for(int i = 0; i < 6; i++){
        ...
        while(isBeforeExit(y_out_vct)){
            ...
            //std::cerr<<"debug step="<< step_count <<" r=" << y_in_vct[0] <<" "<< y_in_vct[1] <<" "<< y_in_vct[2] <<"  nMax ="<<nMax<<std::endl;
      //std::cerr<<"debug  "<< step_count <<"   " << y_in_vct[0] <<" "<< y_in_vct[1] <<" "<< y_in_vct[2] <<std::endl;
        ...
        //std::cerr<<"debug Exit Pl=" << plExitV[0] <<" "<< plExitV[1] <<" "<< plExitV[2]<<" "<< plExitV[3] <<" step_count="<<step_count<<std::endl;
        ...
        if(iN > 1){
            ...
            //std::cerr<<"debug with time diff=" <<diff<<" eps="<<eps<<" step_count="<<step_count<<std::endl;
            t_step = t_st;
            n_steps = step_count;           
            if(diff < eps){
                //that is enough!
