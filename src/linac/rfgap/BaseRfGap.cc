...
void BaseRfGap::trackBunch(Bunch* bunch, double frequency,
                           double E0TL , double phase)
{
  ...
  //std::cout << "debug delta_eKin = " << delta_eKin * 1.0e+03 <<
  //             "    prime_coeff = " << prime_coeff << std::endl;
  ...
  //std::cout << "debug E0TL = " << E0TL << "    kr = " << kr
  //          << "    kappa = " << kappa << std::endl;
  double x, y, r, rp, d_phi;
  double d_rp = kappa *sin(phase);
  //std::cout << "debug BaseRfGap::trackBunch eKin_in = "
  //          << eKin_in * 1.e+03 << "    xp_coeff = "
  //          << prime_coeff << "    x_coeff = " << d_rp
  //          << "    phase ="<< ((phase*180/OrbitConst::PI)+180.)
  //          << std::endl;
  //std::cout << "debug BaseRfGap::trackBunch eKin_in = "
  //          << eKin_in*1.e+3 << "    chargeE0TLsin = "
  //          << chargeE0TLsin << " phase_time_coeff = "
  //          << phase_time_coeff << std::endl;
  double I0, I1;
  for(int i = 0, n = bunch->getSize(); i < n; i++)
  {
    ...
    //bunch->dE(i) = bunch->dE(i) + charge * E0TL *
    //               cos(phase + d_phi) * I0 - delta_eKin;
    //bunch->dE(i) = (bunch->dE(i) / beta -
    //                chargeE0TLsin * d_phi / beta_gap) * beta_out;
    bunch->dE(i) = bunch->dE(i) - chargeE0TLsin * d_phi;
    //transverse focusing
    //d_rp = kappa *sin(phase + d_phi) * 2.0 * I1 / kr;
