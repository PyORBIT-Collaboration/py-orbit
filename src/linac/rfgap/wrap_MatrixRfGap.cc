    ...
    pyORBIT_Object* pyMatrixRfGap = (pyORBIT_Object*) self;
	  ...
	  double frequency, e0tl, phase, ampl;		
		if(!PyArg_ParseTuple(args,"Odddd:trackBunch",&pyBunch,&frequency,&ampl,&e0tl,&phase)){
			ORBIT_MPI_Finalize("PyMatrixRfGap - trackBunch(Bunch* bunch, freq, ampl, E0TL, phase) - parameters are needed.");
		}
		PyObject* pyORBIT_Bunch_Type = wrap_orbit_bunch::getBunchType("Bunch");
		if(!PyObject_IsInstance(pyBunch,pyORBIT_Bunch_Type)){
			ORBIT_MPI_Finalize("PyMatrixRfGap - trackBunch(Bunch* bunch, freq, ampl, E0TL, phase) - first param. should be a Bunch.");
		}
		Bunch* cpp_bunch = (Bunch*) ((pyORBIT_Object*)pyBunch)->cpp_obj;
		cpp_MatrixRfGap->trackBunch(cpp_bunch,frequency,ampl,e0tl,phase);
		Py_INCREF(Py_None);
    return Py_None;	
	}		
